...
**Producer Price Index (PPI).** PPI is one of the flagship series families served here: request PPI `series_id`s (industry `PCU*` and commodity `WPU*` codes) through the same v2 endpoint for input/output producer pricing across steel, chemicals, freight, machinery, packaging, and construction materials, the standard open source for industrial-margin analysis. CPI, employment, and wage series come through the same interface.
