...
- **GitHub is canonical.** The Google Sheet and Airtable base are downstream render targets overwritten on each publish run (upsert on `id` + prune for Airtable). No bidirectional sync. Airtable is one flat `Datasets` table (one row per dataset, `join_keys` inline as a multi-select); the Sheet keeps it as a delimited cell.
